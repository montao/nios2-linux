...
# Read until a timeout occurs. Return "" on timeout.
sub timeout_read {
	my $fh = shift;
	my $timeout = 1.0;
	if (scalar(@_) > 0) {
	...
	while (select($rd, undef, undef, $timeout) != 0) {
		last if sysread($fh, $frag, $BUFSIZ) == 0;
		$result .= $frag;
		$timeout = 0;
	}
	return $result;
	...
	for my $proto (@$protos) {
		if ($proto eq "tcp") {
			...
			server_client_test($desc . " (SSL)",
				[@$server_args, ("--ssl", "--ssl-key", "test-key.pem", "--ssl-cert", "test-cert.pem")], [@$client_args, ("--ssl")], $code);
		} else {
...
sub max_conns_test {
	...
	test $desc, sub {
		...
		for ($i = 0; $i < $count; $i++) {
			...
			if (!$resp) {
				# Try again reading from the client, for --exec.
	...
	for my $proto (@$protos) {
		if ($proto eq "tcp") {
			...
			max_conns_test("$desc (SSL)",
				[@$server_args, ("--ssl", "--ssl-key", "test-key.pem", "--ssl-cert", "test-cert.pem")], [@$client_args, ("--ssl")], $count);
		} else {
			die "Unknown protocol $proto";
		}
	}
...
test "Connection persistence (TCP)",
sub {
	my $resp;
...
test "Connection persistence (UDP)",
sub {
	my $resp;
...
["--proxy-type", "http"], ["--proxy", "$HOST:$PORT", "--proxy-type", "http"], sub {
	...
	!$resp or die "Proxy client sent \"$resp\" to the user stream";
};
...
["--ssl", "--ssl-key", "test-key.pem", "--ssl-cert", "test-cert.pem"], ["--ssl"], sub {
	my $resp;
...
($s_pid, $s_out, $s_in) = ncat_server("--ssl", "--ssl-key", "test-key.pem", "--ssl-cert", "test-cert.pem");
...
# Test --max-conns.
for my $count (0, 1, 10) {
	max_conns_test_tcp_udp_ssl("--max-conns $count", [], [], $count);
}

for my $count (0, 1, 10) {
	max_conns_test_tcp_ssl("--max-conns $count --broker", [], [], $count);
}
