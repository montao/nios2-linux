...
the search data base option at the 'Search options' tab before start the search") % APP_DISPLAY_NAME)
    ...
    def append_result(self, parsed_result):
        ...
        try:
            date = time.localtime(float(parsed_result.start))
            date_field = time.strftime("%Y-%m-%d %H:%M", date)
        except ValueError:
...
class DateSubcriterion(Subcriterion):
    ...
    def set_date(self, date):
        ...
        self.date_button.set_label(time.strftime("%d %b %Y", date))
        self._date = date
