...
void traceopt(char *option)
{
	...
	if (traceopt_count + 1 > traceopt_size) {
		...
		if (traceptr == NULL)
			 printf("Error allocating space for %d trace options\n",
			       traceopt_count+1);
	}
	ptr = malloc(strlen(option)+1);
	if (ptr == NULL)
		printf("error allocating space for trace option %s\n", option);
...
static void process_options (int argc, char *argv[])
{
	...
	for (;;) {
		...
		case 'S':  /* SMP testing */
			if (numa)
				printf("numa and smp options are mutually exclusive\n");
			smp = 1;
			num_threads = max_cpus;
			setaffinity = AFFINITY_USEALL;
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < num_threads; i++) {
		...
		if (status != 0)
			printf("error from pthread_attr_init for thread %d: %s\n", i, strerror(status));
		...
		if (numa) {
			...
			if (pthread_attr_getstack(&attr, &currstk, &stksize))
				printf("failed to get stack size for thread %d\n", i);
			...
			if (pthread_attr_setstack(&attr, stack, stksize))
				printf("failed to set stack addr for thread %d to 0x%x\n",
				      i, stack+stksize);
		}
		...
		if (par == NULL)
			printf("error allocating thread_param struct for thread %d\n", i);
		...
		if (stat == NULL)
			printf("error allocating thread status struct for thread %d\n", i);
		...
		if (histogram) {
			...
			if (stat->hist_array == NULL)
				printf("failed to allocate histogram of size %d on node %d\n",
				      histogram, i);
