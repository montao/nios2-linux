...
static void writeshm(int shmid, char *segptr, char *text)
{
	strcpy(segptr, text);
	printf("Done...\n");
}

static void readshm(int shmid, char *segptr)
{
	printf("segptr: %s\n", segptr);
}
...
static void changemode(int shmid, char *mode)
{
	...
	sscanf(mode, "%o", &scan_mode);
	myshmds.shm_perm.mode = scan_mode;
